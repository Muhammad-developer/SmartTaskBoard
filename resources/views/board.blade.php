...
<div x-data="taskBoard()" x-init="init()" class="min-h-screen p-6 transition-colors duration-200" :class="darkMode ? 'bg-gray-900' : 'bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50'">
    ...
    <!-- New Task Modal - Enhanced Trello-like Interface -->
    <div x-show="showNewTaskModal"
        ...
        <div class="rounded-2xl shadow-2xl max-w-4xl w-full h-auto max-h-[90vh] overflow-y-auto transition-colors"
            ...
            <div class="sticky top-0 flex items-center justify-between p-6 border-b transition-colors"
                ...
                <div>
                    <h2 class="text-2xl font-bold transition-colors" :class="darkMode ? 'text-white' : 'text-gray-900'" x-text="translate('create_task')"></h2>
                    <p class="text-sm transition-colors" :class="darkMode ? 'text-gray-400' : 'text-gray-600'">Organize your work with all the details</p>
                </div>
            ...
            <!-- Main Content - Two Column Layout (Trello-like) -->
            <form @submit.prevent="createTask()" class="grid grid-cols-1 lg:grid-cols-3 divide-y lg:divide-y-0 lg:divide-x transition-colors"
                ...
                <div class="lg:col-span-2 p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <textarea x-model="newTask.description"
                                  rows="6"
                                  placeholder="Add a detailed description of your task. You can add more details later."
                    ...
                    <div>
                        ...
                        <select x-model="newTask.column_id"
                                ...
                                :class="darkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300 text-gray-900'">
                            <option value="">Select a column for this task...</option>
                            <template x-for="column in board.columns" :key="column.id">
                                <option :value="column.id" x-text="`${column.name} (${column.tasks?.length || 0} tasks)`"></option>
                            </template>
                        </select>
                    </div>
                </div>
                ...
                <div class="lg:col-span-1 p-6 space-y-4" :class="darkMode ? 'bg-gray-750' : 'bg-gray-50'">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <template x-for="priority in ['low', 'medium', 'high', 'urgent']" :key="priority">
                                <button type="button"
                                        ...
                                        :class="{
                                            'ring-2 ring-blue-500': newTask.priority === priority,
                                            ...
                                            'opacity-50': newTask.priority !== priority
                                        }"
                                        class="px-3 py-2 rounded-lg font-medium text-xs font-medium transition-all">
                    ...
                    <div>
                        ...
                        <p class="text-xs mt-2 transition-colors" :class="darkMode ? 'text-gray-500' : 'text-gray-600'">
                            <span x-show="newTask.due_date" x-text="`Due: ${new Date(newTask.due_date).toLocaleDateString()}`"></span>
                            <span x-show="!newTask.due_date">No due date set</span>
                        </p>
                    </div>
